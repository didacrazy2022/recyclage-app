"""
Recyclage de proximité : prototype Streamlit.
...
st.set_page_config(page_title="Recyclage de proximité", page_icon="♻️", layout="centered")
...
STATUTS = {
    ...
    "reserve": "🟠 Réservé par un collecteur",
    "collecte": "✅ Récupéré",
}
...
def page_accueil():
    st.title("♻️ Recyclage de proximité")
    st.write(
        "Les familles trient leur plastique, les collecteurs viennent le récupérer devant la porte. "
        "Moins de déchets dans les poubelles, plus de revenus pour les collecteurs."
    )
    ...
    st.markdown(
        "1. **Famille** : je garde mes bouteilles propres dans un sac et je le publie dans l'application.\n"
        "2. **Collecteur** : je vois les sacs près de moi et je réserve ceux que je veux récupérer.\n"
        "3. **Récupération** : la famille donne un code à 4 chiffres au collecteur, qui valide la collecte.\n"
        "4. **Récompense** : la famille gagne des points, le collecteur revend le plastique."
    )

    dispo = query("SELECT lat, lon FROM bags WHERE status='disponible'")
    if dispo:
        st.subheader(f"{len(dispo)} sacs en attente de collecte")
        st.map(pd.DataFrame([dict(r) for r in dispo]))

    st.info("👈 Connecte-toi avec ton numéro de téléphone dans le menu (en haut à gauche sur téléphone).")
...
def page_collecteur(user):
    st.title(f"Bonjour {user['name']} 👋")
    t1, t2, t3 = st.tabs(["🗺️ Sacs disponibles", "🚚 Mes réservations", "📊 Mon bilan"])
    with t1:
...
def mes_reservations(user):
    ...
    for b in sacs:
        with st.container(border=True):
            st.markdown(f"**Sac {b['size'].lower()}** · {b['quartier']}")
            st.write(f"📍 {b['address']}")
            if b["note"]:
                st.write(f"📝 {b['note']}")
            st.write(f"👤 {b['family_name']} · 📞 {b['family_phone']}")
            if b["photo"]:
